...
final class NativeImageGenerateMetadataFiles(targetDirectory: File)(implicit logger: Logger) {
  ...
  def findArtefactVersion(module: Module, version: Option[String]): Option[ArtefactMeta] = {
    val ret = readModuleVersions(module).flatMap {
      versions =>
        versions.entries
          .find {
            entry =>
              version match {
                ...
                case Some(info) =>
                  entry.defaultFor.exists(
                    regex => info.matches(regex)
                  )
              }
          }
  ...
  def findArtefact(module: Module, version: Option[String]): Option[(ArtefactMeta, ArtefactFiles)] = {
    findArtefactVersion(module, version).map {
      meta =>
...
object NativeImageGenerateMetadataFiles {
  def generateResourceFiles(targetDirectory: File, generatedFilesDirectory: File, items: List[ResourceType])(implicit logger: Logger): List[File] = {
    new NativeImageGenerateMetadataFiles(targetDirectory).generateResourceFiles(generatedFilesDirectory, items)
  }

  case class Module(groupId: String, artifactId: String) {
  ...
  object ModuleIndexEntry {
    implicit val json: Codec[ModuleIndexEntry] =
      Codec.forProduct4(
        ...
        "module",
        "requires"
      )(ModuleIndexEntry.apply)(
        m => (m.allowedPackages, m.directory, m.module, m.requires)
      )
  }
  ...
  object ModuleVersionsIndexEntry {
    implicit val json: Codec[ModuleVersionsIndexEntry] =
      Codec.forProduct5(
        ...
        "default-for",
        "tested-versions"
      )(ModuleVersionsIndexEntry.apply)(
        m => (m.latest, m.metadataVersion, m.module, m.defaultFor, m.testedVersions)
      )
  }
  ...
  object ResourcesJsonPatterns {
    implicit val json: Codec[ResourcesJsonPatterns] =
      Codec.forProduct2(
        "includes",
        "excludes"
      )(ResourcesJsonPatterns.apply)(
        m => (m.includes, m.excludes)
      )
  }

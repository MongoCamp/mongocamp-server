...
object MongoDbMetricsPlugin extends ServerPlugin with LazyLogging {
  ...
  override def activate(): Unit = {
    ...
    if (ConfigurationService.getConfigValue[Boolean]("micrometer.mongodb.jvm")) {
      MetricsConfiguration.addJvmRegistry(MongoStepMeterRegistry(databaseProvider.dao("monitoring_jvm"), configMap))
    }
    if (ConfigurationService.getConfigValue[Boolean]("micrometer.mongodb.system")) {
      MetricsConfiguration.addSystemRegistry(MongoStepMeterRegistry(databaseProvider.dao("monitoring_system"), configMap))
    }
    if (ConfigurationService.getConfigValue[Boolean]("micrometer.mongodb.mongo")) {
      MetricsConfiguration.addMongoRegistry(MongoStepMeterRegistry(databaseProvider.dao("monitoring_mongo_db"), configMap))
    }
    if (ConfigurationService.getConfigValue[Boolean]("micrometer.mongodb.event")) {
      MetricsConfiguration.addEventRegistry(MongoStepMeterRegistry(databaseProvider.dao("monitoring_event"), configMap))
    }
  }

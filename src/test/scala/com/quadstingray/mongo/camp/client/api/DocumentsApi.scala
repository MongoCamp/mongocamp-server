...
class DocumentsApi(baseUrl: String) extends CirceSchema {
  ...
  def documentsList(
      apiKey: String,
      bearerToken: String
  )(collectionName: String, rowsPerPage: Option[Long] = None, page: Option[Long] = None) =
    basicRequest
      .method(Method.GET, uri"$baseUrl/mongodb/collections/$collectionName/documents?rowsPerPage=$rowsPerPage&page=$page")
      .contentType("application/json")
      .header("X-AUTH-APIKEY", apiKey)
      .auth
      .bearer(bearerToken)
